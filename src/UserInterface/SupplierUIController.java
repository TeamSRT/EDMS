...
public class SupplierUIController implements Initializable {
    ...
    @FXML
    private void btnShowDetailsOnClicked(ActionEvent event) throws IOException {
         DataManager.selectedSupplier = tvSupplier.getSelectionModel().getSelectedItem();
         if(DataManager.selectedSupplier == null) {
            Alert alert = new Alert(AlertType.ERROR);
            alert.setTitle("Invalid Action");
            alert.setContentText("You need to select a supplier to show his details!");
            alert.show();
        } else {          
            Stage dialog = new Stage();
            dialog.initModality(Modality.APPLICATION_MODAL);
            dialog.initOwner((Stage) ((Node) event.getSource()).getScene().getWindow());
            Parent rootPane = FXMLLoader.load(getClass().getResource("/UserInterface/SupplierUIDetails.fxml"));
